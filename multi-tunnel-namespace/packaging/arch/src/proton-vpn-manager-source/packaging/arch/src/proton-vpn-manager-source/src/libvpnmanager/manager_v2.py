...
class TunnelManager:
    ...
    async def connect_tunnel(
        ...
        Raises:
            ...
            PermissionError: If user doesn't own tunnel and isn't admin
    ...
    async def disconnect_tunnel(self, tunnel_name: str, username: str) -> None:
        ...
        async with self._lock:
            ...
            if tunnel.username != username and not self._is_admin(username):
                raise PermissionError(f"Cannot disconnect tunnel owned by {tunnel.username}")
    ...
    async def destroy_tunnel(self, tunnel_name: str, username: str) -> None:
        ...
        async with self._lock:
            ...
            if tunnel.username != username and not self._is_admin(username):
                raise PermissionError(f"Cannot destroy tunnel owned by {tunnel.username}")
        ...
        async with self._lock:
            ...
            if all_users:
                if not self._is_admin(username):
                    raise PermissionError("Only admins can list all users' tunnels")
                return tunnels
    ...
    async def get_tunnel(self, tunnel_name: str, username: Optional[str] = None) -> Optional[Tunnel]:
        ...
        async with self._lock:
            ...
            if tunnel and username:
                if tunnel.username != username and not self._is_admin(username):
                    raise PermissionError(f"Tunnel belongs to {tunnel.username}")
            return tunnel
